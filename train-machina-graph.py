...
def visual_up_per():
    ...
    def figure_draw(znach, name_):
        ...
        lines_oborud(gk, 'white', type_='UP')
        lines_oborud(tt, 'lime', type_='UP')
...
def visual_front():
    ...
    for no in SZ.keys():
        ...
        for f in harm.keys():
            ...
            point_draw(data, y_ln, z_ln, '', y_lb=str(f), lv=0)
        plt.suptitle(name)
        ...
        for nam, val in koridor.items():
            plt.subplot(2, 2, i)
            point_draw(val, y_ln, z_ln, '', lv=0)
            if 'кабина' not in SZ[no]:
            ...
            if 'переменный' in nam:
                if 'ВУ_СР ближние' in SZ[no]:
                    ...
                    lines_oborud(gk, 'white')
                    lines_oborud(tt, 'lime')
                    lines_oborud(pr, 'pink')
                elif 'ВУ_СР дальние' in SZ[no]:
                    lines_shina(mednaya_truba, 'blue')
                    lines_oborud(vu, 'aqua')
                else:
                    kab_lines_front()
            else:
...
for nm, zn in chel.items():
    print(nm)
    print(f'поле \t\t| {zn[0]:.4f}\t| {zn[1]:.4f} \t|')
    print(f'доля погл. \t| {zn[0] * ti * S * p / b:.6f} \t| {zn[1] * ti * S * p / b:.6f} \t|')
